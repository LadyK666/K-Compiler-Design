...
"""
语义分析测试自动化脚本
自动测试 semantic_analysis_tests 文件夹下的所有测试文件
将结果输出到 CSV 文件
"""
...
# 测试文件夹路径
TEST_DIR = "semantic_analysis_tests"
...
# CSV输出文件名
OUTPUT_CSV = f"semantic_analysis_test_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def extract_semantic_analysis(output):
    """从完整输出中提取语义分析部分"""
    if not output:
        return ""
    ...
    # 方法1: 查找从第一个 [符号表] 开始到 [语义检查完成] 结束的内容
    # 语义分析在AST输出之后，TAC生成之前
    pattern1 = r'(\[符号表\].*?\[语义检查完成\].*?)(?=\n===== TAC|\n=====.*?=====|\n========================================|$)'
    match1 = re.search(pattern1, output, re.DOTALL)
    if match1:
        return match1.group(1).strip()
    
    # 方法2: 如果没找到完整的，分别查找符号表和语义检查完成
    # 先找符号表部分
    pattern2a = r'(\[符号表\].*?)(?=\n\[语义检查完成\]|\n===== TAC|\n=====.*?=====|\n========================================|$)'
    match2a = re.search(pattern2a, output, re.DOTALL)
    
    # 再找语义检查完成部分
    pattern2b = r'(\[语义检查完成\].*?)(?=\n===== TAC|\n=====.*?=====|\n========================================|$)'
    match2b = re.search(pattern2b, output, re.DOTALL)
    
    if match2a and match2b:
        # 合并两部分
        return (match2a.group(1) + '\n' + match2b.group(1)).strip()
    elif match2a:
        return match2a.group(1).strip()
    elif match2b:
        return match2b.group(1).strip()
    
    # 方法3: 查找语义错误信息（可能没有符号表输出）
    pattern3 = r'(\[语义错误\].*?)(?=\n\[语义检查完成\]|\n===== TAC|\n=====.*?=====|\n========================================|$)'
    match3 = re.search(pattern3, output, re.DOTALL)
    if match3:
        semantic_errors = match3.group(1).strip()
        # 尝试找到语义检查完成信息
        if match2b:
...
def main():
    ...
    for idx, (file_path, test_dir, file_name) in enumerate(test_files, 1):
        ...
        # 提取语义分析部分
        semantic_output = extract_semantic_analysis(full_output)
        
        # 调试输出
        if semantic_output:
            output_preview = semantic_output[:100].replace('\n', ' ') if semantic_output else "(无语义分析输出)"
            print(f"  语义分析输出预览: {output_preview}...")
        else:
            print(f"  警告: 未找到语义分析输出")
            if full_output:
                print(f"  完整输出前100字符: {full_output[:100]}...")
        
        # 判断测试状态
        # 对于语义分析，返回码非0或包含语义错误都算失败（对于error_tests，有错误是正常的）
        if test_dir == "error_tests":
            # 错误测试：应该包含语义错误
            if "[语义错误]" in semantic_output or "[语义检查完成]" in semantic_output:
                status = "成功（检测到错误）"
                success_count += 1
            else:
                status = "失败（未检测到错误）"
            ...
            # 正常测试：不应该有语义错误
            if return_code == 0 and "[语义错误]" not in semantic_output and "[语义检查完成]" in semantic_output:
                status = "成功"
                success_count += 1
            else:
                status = "失败/错误"
        ...
        csv_data.append([
            ...
            rel_path,
            file_content,
            semantic_output,  # 只保存语义分析部分
            return_code,
            status
        ])
        
        # 显示输出长度（用于调试）
        output_len = len(semantic_output) if semantic_output else 0
        print(f"  状态: {status} (返回码: {return_code}, 语义分析输出长度: {output_len} 字符)")
