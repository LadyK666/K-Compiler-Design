...
"""
语法分析测试自动化脚本
自动测试 syntax_analysis_tests 文件夹下的所有测试文件
将结果输出到 CSV 文件
"""
...
# 测试文件夹路径
TEST_DIR = "syntax_analysis_tests"
...
# CSV输出文件名
OUTPUT_CSV = f"syntax_analysis_test_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def extract_syntax_analysis(output):
    """从完整输出中提取语法分析部分（AST输出）"""
    if not output:
        return ""
    
    # 根据实际输出格式：
    # 1. 词法分析结果以 "========================\n" 结束
    # 2. AST输出（语法分析）在词法分析结果之后，直接是AST节点输出
    # 3. 语义分析以 "[符号表]" 开始
    
    # 方法1: 精确匹配词法分析结果结束后的AST部分
    # 从 "========================" 之后到 "[符号表]" 之前
    pattern1 = r'========================\n+(.*?)(?=\n\[符号表\]|\n===== TAC|\n========================================|$)'
    match1 = re.search(pattern1, output, re.DOTALL)
    if match1:
        potential_output = match1.group(1).strip()
        
        # 排除词法分析的关键词
        lexical_keywords = ['词法分析', 'Token', 'token', '行号', 'Token类型', 'Token值', '词素', '共识别', 'TYPE_', 'ID', 'SEMI', 'INT', 'FLOAT', 'CHAR', 'ASSIGNOP', 'PLUS', 'MINUS']
        
        # 如果包含词法分析关键词，需要找到第一个AST节点开始的位置
        if any(keyword in potential_output for keyword in lexical_keywords):
            # 查找第一个AST特征出现的位置
            ast_keywords = ['变量声明', '数组声明', '条件语句', '循环语句', '输入变量', '输出表达式', '表达式语句', '复合语句']
            for keyword in ast_keywords:
                idx = potential_output.find(keyword)
                if idx >= 0:
                    # 从第一个AST关键词开始提取到语义分析开始
                    ast_output = potential_output[idx:].strip()
                    # 移除可能包含的语义分析内容
                    ast_output = re.sub(r'\n\[符号表\].*$', '', ast_output, flags=re.DOTALL).strip()
                    if ast_output:
                        return ast_output
        else:
            # 不包含词法分析关键词，直接返回
    ...
    # 方法2: 直接查找包含AST特征的行，确保不在词法分析区域内
    # 从第一个AST节点开始到语义分析或TAC开始
    pattern2 = r'((?:变量声明|数组声明|条件语句|循环语句|输入变量|输出表达式|表达式语句|复合语句|单目-).*?)(?=\n\[符号表\]|\n.*语义分析|\n===== TAC|\n========================================|$)'
    match2 = re.search(pattern2, output, re.DOTALL)
    if match2:
        ast_output = match2.group(1).strip()
        # 确保不包含词法分析内容
        if not any(keyword in ast_output for keyword in ['词法分析', 'Token', 'token', '行号', 'Token类型', 'Token值', '词素', '共识别']):
            return ast_output
    
    # 方法3: 查找语法错误信息（Grammar Error）
    grammar_error_pattern = r'Grammar Error.*?(?=\n==========|\n=========================|\n========================================|$)'
    grammar_error_match = re.search(grammar_error_pattern, output, re.DOTALL)
    if grammar_error_match:
        return grammar_error_match.group(0).strip()
...
def main():
    ...
    for idx, (file_path, test_dir, file_name) in enumerate(test_files, 1):
        ...
        # 提取语法分析部分
        syntax_output = extract_syntax_analysis(full_output)
        
        # 调试输出
        if syntax_output:
            output_preview = syntax_output[:100].replace('\n', ' ') if syntax_output else "(无语法分析输出)"
            print(f"  语法分析输出预览: {output_preview}...")
        else:
            print(f"  警告: 未找到语法分析输出")
            if full_output:
                print(f"  完整输出前100字符: {full_output[:100]}...")
        
        # 判断测试状态
        # 对于语法分析，返回码非0或找到语法错误都算失败
        if return_code == 0 and "Grammar Error" not in full_output:
            status = "成功"
            success_count += 1
        else:
            status = "失败/错误"
        ...
        csv_data.append([
            ...
            rel_path,
            file_content,
            syntax_output,  # 只保存语法分析部分
            return_code,
            status
        ])
        
        # 显示输出长度（用于调试）
        output_len = len(syntax_output) if syntax_output else 0
        print(f"  状态: {status} (返回码: {return_code}, 语法分析输出长度: {output_len} 字符)")
